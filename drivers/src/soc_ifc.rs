...
pub fn reset_reason() -> ResetReason {
    ...
    let fw_upd = soc_ifc_regs.cptra_reset_reason().read().fw_upd_reset();
    let warm_rst = soc_ifc_regs.cptra_reset_reason().read().warm_reset();

    // Prioritize firmware hitless update to ensure staged runtime activation completes
    if fw_upd {
